...
- 迁移指引（破坏性变更）：
  ...
  - 后续均使用 `mcp`，例如：`mcp apply-cli ...`、`mcp ide-all`、`mcp run ...`、`mcp check --probe`。


## v1.2.2 (2025-11-06)

- 行为变更：安装脚本默认不再自动同步/落地 MCP，仅生成统一清单并执行体检；需要落地时使用 `mcp pick/apply-cli/run` 或可选 `scripts/mcp-sync.sh`。
...
- 文档：
 ...
 - CLI：`mcp` 新增 `-n/--dry-run` 预览模式，支持 `apply-cli`/`ide-all`/`run` 等子命令仅显示将进行的写入、注册与启动动作，不做实际修改。
 - 新增别名：已移除（统一使用 `mcp`）。
...
- CLI：`mcp ide-all` 增加确认提示；`apply-cli --client claude` 强制重注册，确保按中央清单收敛到 npx @latest（`serena` 仍本地二进制）。
...
3) 精准落地（按需）：
   - 仅对需要的客户端下发所需 MCP，例如：
     ...
     - `mcp apply-cli --client claude --servers filesystem,playwright`
4) 验证：
