...
def _clear_claude_project_overrides(dry_run: bool = False) -> int:
    ...
    obj = U.load_json(p, {}, "Claude项目配置读取")
...
def _clear_json_map(
    label: str, path: Path, top_key="mcpServers", extra: str | None = None, dry_run: bool = False
):
    if dry_run:
...
def run(args) -> int:
    ...
    if os.environ.get("MCP_CLEAR_YES") == "1":
        ...
        reply = (
            input(f"将清空以下客户端: {', '.join(targets)}，确认? [y/N]: ").strip().lower() or "n"
        )
    if reply != "y":
        print("已取消")
        return 0
    ...
    for t in targets:
        try:
            if t == "claude":
                ...
                _clear_claude_project_overrides(dry_run=dry_run)
                ...
                _clear_json_map(
                    "Gemini",
                    U.HOME / ".gemini" / "settings.json",
                    "mcpServers",
                    extra="gemini.allowed",
                    dry_run=dry_run,
                )
            elif t == "iflow":
                _clear_json_map(
                    "iFlow", U.HOME / ".iflow" / "settings.json", "mcpServers", dry_run=dry_run
                )
            elif t == "droid":
                _clear_json_map(
                    "Droid", U.HOME / ".factory" / "mcp.json", "mcpServers", dry_run=dry_run
                )
            elif t == "cursor":
                _clear_json_map(
                    "Cursor", U.HOME / ".cursor" / "mcp.json", "mcpServers", dry_run=dry_run
                )
            elif t == "vscode-user":
                _clear_json_map("VS Code(User)", U._vscode_user_path(), "servers", dry_run=dry_run)
            elif t == "vscode-insiders":
                _clear_json_map(
                    "VS Code(Insiders)", U._vscode_insiders_path(), "servers", dry_run=dry_run
                )
