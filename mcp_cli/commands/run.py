...
def apply_json_map(
    label: str, path: Path, subset: dict, top_key: str = "mcpServers", dry_run: bool = False
) -> int:
    ...
    if label == "Droid":
        # 保留非 MCP 字段，仅覆盖 mcpServers，避免误伤其它配置
...
def apply_claude(subset: dict, verbose: bool = False, dry_run: bool = False) -> int:
    # 文件端
    apply_json_map(
        "Claude(文件)", U.HOME / ".claude" / "settings.json", subset, "mcpServers", dry_run=dry_run
    )
    ...
    if remove_ok or remove_fail:
        print(
            ...
            + (f"; failed=[{', '.join(remove_fail)}]" if remove_fail else "")
        )
    if add_ok or add_fail:
        print(
            f"  add   : ok={len(add_ok)} fail={len(add_fail)}"
            + (f"; failed=[{', '.join(add_fail)}]" if add_fail else "")
        )
    return 0
...
def run(args) -> int:
    ...
    2) 预选模式：当 args 上已经带有 `client` 与 `servers` 或 `preset` 时直接按给定集合落地，
       避免重复交互。
    """
    ...
    if client and (pre_servers or pre_preset):
        _, servers_all = U.load_central_servers()
        servers, disabled = U.split_enabled_servers(servers_all)
        if pre_servers:
        ...
        disabled_sel = [n for n in names if n in disabled]
        missing = [n for n in names if (n not in servers and n not in disabled)]
        if disabled_sel:
            print(
                "[ERR] 选定的服务器在中央清单中已被禁用 (enabled: false): "
                + ", ".join(disabled_sel)
            )
            return 1
        if missing:
        ...
        if (
            pick_preset_raw
            and pick_preset_raw.isdigit()
            and 1 <= int(pick_preset_raw) <= len(presets)
        ):
            sel_name = presets[int(pick_preset_raw) - 1][0]
            preset_servers = PRESET_PACKS.get(sel_name, {}).get("servers", [])
            disabled_sel = [n for n in preset_servers if n in disabled]
            missing = [n for n in preset_servers if (n not in servers and n not in disabled)]
            if disabled_sel:
                print(
                    f"[WARN] 预设 {sel_name} 中部分条目已在 central 禁用 (enabled:false)，已跳过: "
                    + ", ".join(disabled_sel)
                )
            if missing:
                print(
                    f"[WARN] 预设 {sel_name} 中部分条目不在中央清单，已跳过: {', '.join(missing)}"
                )
    ...
    if client == "claude":
        ...
        rc = apply_json_map(
            "Gemini", U.HOME / ".gemini" / "settings.json", subset, "mcpServers", dry_run=dry_run
        )
    elif client == "iflow":
        rc = apply_json_map(
            "iFlow", U.HOME / ".iflow" / "settings.json", subset, "mcpServers", dry_run=dry_run
        )
    elif client == "droid":
        rc = apply_json_map(
            "Droid", U.HOME / ".factory" / "mcp.json", subset, "mcpServers", dry_run=dry_run
        )
        # 对齐 Droid 注册表
        ...
        rc = apply_json_map(
            "Cursor", U.HOME / ".cursor" / "mcp.json", subset, "mcpServers", dry_run=dry_run
        )
    elif client == "vscode-user":
        rc = apply_json_map(
            "VS Code(User)", U._vscode_user_path(), subset, "servers", dry_run=dry_run
        )
    elif client == "vscode-insiders":
        rc = apply_json_map(
            "VS Code(Insiders)", U._vscode_insiders_path(), subset, "servers", dry_run=dry_run
        )
    else:
        print("[ERR] 未知 client")
        return 2
    ...
    # 预选/DRY-RUN 模式默认不进入“启动命令”交互，避免脚本/自动化卡住
    if dry_run or preselected:
        print("[OK] 已应用所选集合；已跳过启动步骤。")
        return 0
